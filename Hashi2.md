...
| 11 | OrderDAOTest | testCancelOrder1() | Standard: PENDING order → cancelOrder() → returns true, DB status = CANCELLED. |
| 12 | OrderDAOTest | testCancelOrder2() | Exception: non-existing orderId → returns false. |
...
### 2.4. JUnit test case code for cancel order

```java
    // Test 11: cancelOrder - standard (PENDING order)
    @Test
    public void testCancelOrder1() {
        OrderDAO dao = new OrderDAO();
        UUID pendingOrderId = UUID.fromString("<UUID-1>"); // CS-0001, PENDING
    ...
    // Test 12: cancelOrder - non-existing orderId
    @Test
...
### 2.5. JUnit test case code for update order status
...
| 3 | Cancel order | Admin cancels a PENDING order. |
| 4 | Cancel order | Admin attempts to change status of an already CANCELLED order. |
| 5 | Cancel order | Admin attempts to change status of a COMPLETED order. |
